"""run25 STEP 1 GATE: the base controller ALONE must lap the NEW grip-aware profile (V_MAX 55 +
T11 cut), cleanly -- braking from the higher straight speeds in time and surviving T11 at the
reduced target, no spin. Same must-lap gate as run23 step 1. Drives BaseController (no policy, no
training) from the start line; logs the full lap + a per-corner table (slope / v_target / entry &
apex speed) + a speed map. Port 25252 (sim free). steer_rate=0.5."""
import math, os, sys
...
def main():
    ...
    print(f"GATE: controller alone on the NEW profile (V_MAX={V_MAX:.0f}). lap=4326m, T1 exit 394m\n")
    ...
    ax.set_title(f"run25 gate: controller on NEW profile -> {lap}, max_arc={ma:.0f}m, peak {peak*3.6:.0f} kph")
    fig.savefig("docs/run25_gate_map.png", dpi=110); print("\nwrote docs/run25_gate_map.png")
    env.close()
