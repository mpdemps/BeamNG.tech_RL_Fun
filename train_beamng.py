...
class TBEvalCallback(EvalCallback):
    ...
    _MEAN_KEYS = ("mean_speed", "max_arc", "over_speed_frac", "beta_mean", "beta_p90",
                  ...
                  "r_backward", "backward_frac", "straight_slip_frac",  # run28/29 reverse + straight-slip
                  "residual_abs", "residual_abs_steer", "residual_abs_throttle",
                  "residual_throttle_pos", "residual_throttle_satfrac",
                  "residual_cf_mean",                                # run31 corner-gate telemetry
                  )  # run22/24: mean |applied
...
def main():
    ...
    if args.drift:
        monitor_info_keys = monitor_info_keys + ("r_drift", "beta_err_mean", "drift_corner_steps",
                                                 "r_backward", "backward_frac", "straight_slip_frac")
    if args.residual and args.drift:
        monitor_info_keys = monitor_info_keys + ("residual_cf_mean",)  # run31 corner-gate telemetry
    ...
    ckpt_callback = CheckpointCallback(
        ...
        # run31: prefix with the RUN NAME (was "rolling") so checkpoint files are globally unique
        # (e.g. mikey_run31_140000_steps.zip) -- Taildropped files from different runs used to
        # collide on identical rolling_<step>_steps.zip names. auto_restart_train's freeze-recovery
        # glob matches *_steps.zip, prefix-agnostic.
